# -*- coding: utf-8 -*-
"""Python objects to parse and process the DENSITY-D1M data.

The data file contains the predictions of the deformed density distribution obtained
within the Hartree-Fock-Bogoliubov method with the D1M Gogny effective 
interaction [1 and references therein]. The axial densities are expanded in terms 
of spherical harmonics $Y_{lambda0}$. The 4 first multipoles are provided.

The present D1M compilation of neutron and proton densities includes all nuclei
with N,Z>=8 and Z<=110 between the proton and neutron driplines. 
The densities are tabulated in the radius grid in steps of 0.1 fm up to a 
maximum value of 20 fm.
...
local_data_dir = _os.path.join("masses", "Density-d1m")
...
class Entry(_db.NuclideDatabaseEntry):
    """An entry in the DENSITY-D1M database.

    Contains radial density distributions from the D1M Gogny HFB model,
...
def read_ascii_file(filepath: str) -> dict:
    """
    Parse the DENSITY-D1M data file and return a dictionary of Entry objects.
...
class Database(_db.Database):
    """A database for the density-D1M data."""
