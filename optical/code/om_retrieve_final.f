...
! 28/04/2008      0.4             R.Capote         RIPL-3 version
!---------------------------------------------------------------
!
! AUTHOR(S) INFORMATIONS:
!
c     Version date: April 2008, RCN
c
c     Some details and protections added based on P.Talou validation.
c------------------------------------------------------------------------
c     Version date: November 2007, RCN
c
c     Volume integrals are calculated (modtyp <= 2)
c     ECIS06 input for dispersive potentials implemented
c
c------------------------------------------------------------------------
...
c       ecis.inp  = standard input file for the ECIS code (modtyp=2)
c       ecisd.inp = dispersive input file for the ECIS06 code (modtyp=5,6)
...
c       iabs(modtyp)= 5 to generate ECIS input file (ecisd.inp) for dispersive 
c                       potentials with externally calculated corrections 
c                       (recommended option for dispersive potentials)
c       iabs(modtyp)= 6 to generate ECIS input file (ecisd.inp) for dispersive 
c                       potentials with ECIS calculated corrections 
c
...
C     parameter (ndim1=10, ndim2=11, ndim3=25, ndim4=30, ndim5=10,
...
c     Change for ominput18.f (1/15/2002)
...
c
     ...
     + '(" Only OPTMAN code can be used, set modtyp = -4")')
      ...
      if(idr.eq.0.and. ecis  .and. modtyp.gt.2) then
       write(6, 
     ...
     + '(" Set modtyp to -2")')
      ...
      if(idr.le.-2.and. ecis  .and. modtyp.gt.2) then
       write(6, 
...
c     Check for Gaussian form factor for WD
 ...
 151  continue
      go to 153
 152  write(6,'(" No ECIS inputs made when Gaussian form factor ",
...
c------------------ RCN1
c         if(pot(m,j,20).ne.0) then
c            write (*,*) iref,izmin,izmax,' Cviso=',sngl(pot(m,j,20))
c            write (*,*) iref,iamin,iamax
c            write (*,*) 
c         endif
c------------------ RCN1
...
C--------------------------------------------------------------------
C     RCN, 08/2004, to handle new extension to the OMP RIPL-2 format
     *       + rco(i,j,2)*(el-VCshift)
     *       + rco(i,j,12)*(el-VCshift)*(el-VCshift)
      else
C     RCN, 09/2004, to handle new extension to the OMP RIPL-2 format
     ...
     *           ( elf**nn + ( rco(i,j,5) + rco(i,j,6)*atar )**nn ) )
      ...
      asc(ii)=abs(aco(i,j,1)) + aco(i,j,2)*(el-VCshift) + aco(i,j,3)*eta
...
c     ROUTINE TO PREPARE AND WRITE OPTMAN INPUT
     ...
     +l with this potential dependence (Contact R.CapoteNoy@iaea.org)'  
      ...
      if (idr.eq.0 .and. izproj.ne.0) then
        write(ko,*) 
     +   'OPTMAN cant not calculate non-dispersive CC proton OMPs'
        stop 6
      endif
...
C     RCN, 09/2004, to handle new extension to the OMP RIPL-2 format
     ...
     *           ( elf**nn + ( rco(i,j,5) + rco(i,j,6)*atar )**nn ) )
      ...
      alib(i)=abs(aco(i,j,1)) + aco(i,j,2)*(el-VCshift) + aco(i,j,3)*eta
     *        + aco(i,j,4)/atar + aco(i,j,5)/sqrt(atar)
     *        + aco(i,j,6)*atar**(2./3.) + aco(i,j,7)*atar
     *        + aco(i,j,8)*atar**2 + aco(i,j,9)*atar**3
     *        + aco(i,j,10)*atar**(1./3.) + aco(i,j,11)*atar**(-1./3.)
c
      RStep = min( 0.25d0, 0.25d0*alib(i) )
...
C       VSOnum = vlib(5)
        ...
        DWScor = DWS + VScoul
        DSOcor = DWVso
C       if(idr.ne.0 .and. rlib(1).eq.rlib(2) )
C    +     vlib(1)= vlib(1) + DWVcor

        if(idr.gt.0) vlib(1)= vlib(1) + DWVcor
...
C
     ...
     +    abs(2*sqrt(eta/atar)*Visov *VHFnum/AHF),
     +    abs(2*sqrt(eta/atar)*WSisov*wd/AS),
     +    abs(2*sqrt(eta/atar)*WSisov*DWScor/AS),
     +    abs(2*sqrt(eta/atar)*WVisov*w/AAV),
     +    abs(2*sqrt(eta/atar)*WVisov*DWVcor/AAV), 
     +    wd,  w,  el
        else
          write(35,'(f7.3,f7.3,f6.3,f6.3,f7.3,
...
c
     ...
     +          "  VSpn   WVpn     VVpn  WS(Lane)   WD      WV(Lane) ",   
     +          "  WV       E" )')    
      if(idr.ne.0 .and. modtyp.le.2) then
...
c
     ...
     >   (LAMBDA,BETA,MAX_LAMBDA,vdd,rdd,add,KEY) !INPUT

      IMPLICIT real*8 (A-H,O-Z)
      DIMENSION BETA(MAX_LAMBDA)
...
C
      DO J=1,(n1+1)/2
        ...
        SUMM=0.d0
        DO L=1,MAX_LAMBDA
          if(beta(L).eq.0.d0) cycle
          SUMP=SUMP+SQRT((2.*L+1.)/4./PI)*PLGNDR(L,0,XP)*beta(L)
          SUMM=SUMM+SQRT((2.*L+1.)/4./PI)*PLGNDR(L,0,XM)*beta(L)
        ENDDO
C
