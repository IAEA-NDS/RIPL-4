...
c     the RIPL-2 optical model potential (omp) library.  The code
c     pemits optional sorting on [projectile, Z, A, Emin] .
c
...
c
c     NOTE: for sorting to work properly, code must be compiled using
c     double precision, that is:
c         f77 om-summary.f -dbl -C -o omsummary
c
c     INPUT FILE
...
c
      ...
      character*1 ians
c
      open(unit=1,file='omparameter.dat')
...
c
 ...
 100  continue
      ...
      do 108 j=1,jcoul
 108  read(ki,*) ecoul(j),rcoul0(j),rcoul(j),rcoul1(j),rcoul2(j),beta(j)
...
c
     ...
     3 lrefx(ndim8),irelx(ndim8),isrt(ndim8),jsrt(ndim8)
...
c     Set sorting criteria
      isrt(i1)=100000*idx(i1)+1000*izmin+iamin
      isrt(i1)=10000*isrt(i1)+iemax
c
...
c
      parameter (ndim8=1000,jcount=40)
...
c
     ...
     3 lrefx(ndim8),irelx(ndim8),isrt(ndim8),jsrt(ndim8)
      ...
      data com/','/,da/'-'/,bl/' '/
      data (mod(i),i=1,4)/'spher. ','CC rot.','vibra. ','N/A def'/
c
...
c
      ...
      if(imodelx(i1).eq.1)model='coupled-channels rotational model'
      if(imodelx(i1).eq.2)model='vibrational model'
      if(imodelx(i1).eq.3)model='non=axial deformed model'
c
