...
c
     ...
     + OPT_ko(ndim6,ndim4),OPT_nca(ndim6,ndim4),OPT_numb(ndim6,ndim4)
...
c
ccc     New type support.   dmart 06.10.2021
 120  if(imodel.le.5)go to 122
...
c
     ...
     +3 DWBA input from a rotational potential.")')
      ...
      if(imodel.eq.4) write(6,'(" You are making an input from a rigid-s
     +oft rotor potential.")')

ccc     New type support.   dmart 06.10.2021
      if(imodel.eq.5) write(6,'(" You are making an input from a deforme
     +d soft rotor potential.")')
ccc
      
      if(imodel.ge.3 .and. modtyp.ne.4) then
       if (imodel.eq.3) write(6, 
     + '(" You are making an input from a soft-rotor potential.")')
       if (imodel.eq.4) write(6, 
     + '(" You are making an input from a rigid-soft potential.")')
       if (imodel.eq.5) write(6, 
     + '(" You are making an input from a deformed soft rotor potential.
     +")')
...
c
c     soft rotor model being used (imodel =3, 4, or 5) 
...
C     Reading vibrational rotor parameters    
 ...
 150  if(imodel.ne.3 .and. imodel.ne.5)go to 170 !New type support.  dmart 06.10.2021
...
ccc     New type support.   dmart 06.10.2021
          ...
          read(ki,*) iz(n),ia(n),ncoll(n),lmax(n),idef(n),bandk(n),
     +    (def(n,k),k=2,idef(n),2) ! also defeormations are read
ccc          
        endif
...
C     Reading rigid-soft rotor parameters    
     ...
     +    (def(n,k),k=2,idef(n),2)
        ...
        do 172 k=1,ncoll(n)
          read(ki,*) exv(k,n),spinv(k,n),iparv(k,n),
     +               SR_ntu(k,n),SR_nnb(k,n),SR_nng(k,n),SR_nno(k,n),
!          upper line is wrong, but left for  backward compatibility
!          lower is correct but requires to fix excess zeros from the potentials
!     +                 OPT_ko(k,n),OPT_nca(k,n),OPT_numb(k,n),
     +               defv(k,n),defr(k,n)
C         write(*,*) iz(n),ia(n),ex(k,n)
          if(exv(k,n).gt.100.d0 .and. nlvmax.eq.0) nlvmax = k-1
 172    continue
...
c     ROUTINE TO PREPARE AND WRITE OPTMAN INPUT
      ...
      real*8 CAVRss,CARRss,CAARss,CARDss,CAACss,CAWDss!,CAWDWss
	integer k

ccc     New type support.   dmart 06.10.2021
      if((imodel.eq.1 .or. imodel.eq.3 .or. imodel.eq.4.or.imodel.eq.5)
     +  .and. idr.ne.0  .and. pot(1,1,24).ne.1.d0 ) then
     ...
     +l with this potential dependence (Contact R.CapoteNoy@iaea.org)'  
      ...
      if (imodel.eq.2) then
        ...
        write(* ,*) 'OPTMAN cant not calculate CC vibrational model'
        stop 6
      endif
...
ccc     New type support.   dmart 06.10.2021
      if((imodel.eq.1 .or. imodel.eq.3 .or. imodel.eq.4.or.imodel.eq.5) 
     +.and.idr.ne.0  .and. nint(pot(2,1,13)).ne.nint(pot(4,1,13)) ) then
...
ccc     New type support.   dmart 06.10.2021
      if(imodel.eq.5) then ! soft deformed rotor (5)
        mepot = 1
        meham = 5
        mesol = 2
        mesha = 2
...
ccc     New control flags in OPTMAN.   dmart 06.10.2021      
      medef = 0
      meaxi = 0
      merad = 0
ccc      
ccc     New type support.   dmart 06.10.2021
      if(imodel.eq.5) then
        medef = 1 ! "effective" deformations from SRM
        if(mod(iatar,2).eq.0.and.mod(iztar,2).eq.0) 
     +     meaxi = 1 ! nonaxial coefficiens for even-even targets
        merad = 1 ! static radius correction (enabled in the last potential)
      endif
ccc
...
ccc     New type support.   dmart 06.10.2021
      ELSEIF(imodel.eq.5) then
        write(ko,'(f10.5,"-",f10.5," MeV ",a8," on ",i3,a2,
     + ": deformed soft rotor, OMP ",14a1," RIPL REF#=",i5)') 
     + en(1),en(ne),parname(ipsc),iatar,nuc(iztar),opname,iref
ccc        
      ENDIF 
...
ccc     New control flags in OPTMAN.   dmart 06.10.2021
     +meapp,mevol,merel,mecul,merzz,merrr,medis,merip,medef,meaxi,merad!0,0,0,0
C
...
C
     ...
     +    SR_dpar(ntar),SR_gshape(ntar)
      ...
      las = 8
      if(imodel.eq.1 .or. imodel.eq.4 .or. imodel.eq.5) npd = idef(ntar)
...
C             Isobar analogue states (for p,n scattering) 
     ...
     +                            nint(2*spin(1,ntar)), 1
          ...
          if(imodel.eq.3) then ! soft rotor
     ...
     +            SR_nng(k,ntar), SR_nno(k,ntar) 
            enddo
          elseif(imodel.eq.4) then   ! rigid-soft rotor
...
C             write(ko,'(e12.5,5I2,2E12.5)') ex(k,ntar),
              write(ko,'(F11.7,1x,5I2,2E12.5)') exv(k,ntar),
     +         nint(2*spinv(k,ntar)),iparv(k,ntar),
     +            SR_ntu(k,ntar), SR_nnb(k,ntar),
     +            SR_nng(k,ntar), defv(k,ntar), defr(k,ntar) ! SR_nno(k,ntar) 
               ! SR_nno(k,ntar) is always zero in the rigid rotor model      
!           Three upper lines are incorrect but left for backward compatibility 
!           Two lower lines are correct but require change of potentials  
...
!     +         defv(k,ntar), defr(k,ntar)
          ...
          elseif(imodel.eq.5) then      
            do k=1,ncoll(ntar)
C             write(ko,'(e12.5,5I2,2E12.5)') ex(k,ntar),
              write(ko,'(F11.7,1x,5I2,2F12.7,4I2)') exv(k,ntar),
     +         nint(2*spinv(k,ntar)),iparv(k,ntar),
     +         OPT_ko(k,ntar),OPT_nca(k,ntar),OPT_numb(k,ntar),
...
C     AC0=ACG+CAAC*dtmp
      ...
      IF(iaref.gt.0) THEN
        ...
        CAWDss = pot(4,1,22) ! Imag sur potential
!        CAWDWss= 0           ! Imag sur potential disp width (not used!)
      ENDIF 
C
...
C       This line is new for OPTMAN v.12 (from November 2011)
C       Updated in October 2021
        if(iaref.eq.0) then
          write(ko,'(6F12.5)') CAACss, dble(iatar), CAWDss!, CAWDWss
        else
          write(ko,'(6F12.5)') CAACss, dble(iaref), CAWDss!, CAWDWss
        endif
ccc     New type support.   dmart 06.10.2021        
        if(meham.eq.1 .or. imodel.eq.4 .or. imodel.eq.5) ! rigid or soft deformed rotor
...
c             variables
	...
	integer k,kecis
     ...
     +                DWScor,ALpha,RSTep
      ...
      RStep = 10.d0
c
c     Generate optical model parameters for ECIS/OPTMAN
c
      pi=acos(-1.d0)
      rc=0.d0
...
c
	...
	wso=vlib(6)*gamma
	  ...
	  write(*,*) 'GS Dispers. OMP parameters'
	  write(*,*) 'E=',sngl(El),' Ef=',sngl(Ef),' Ep=',sngl(Ep)
	  write(*,*) 'Real HF volume  VHF,rHF,aHF:',
     >    sngl(VHFnum*gamma),sngl(rlib(1)),sngl(alib(1))
	  write(*,*) 'Disper. volume  DWV,rWV,aWV:',
     >    sngl(DWV*gamma),sngl(rlib(2)),sngl(alib(2))
	  write(*,*) '         DWV(sym), DW+, DW-:',
     >    sngl((DWV-(Dwplus + Dwmin))*gamma),
     >    sngl(Dwplus*gamma),sngl(Dwmin*gamma)
	  write(*,*) 'Imag volume  wv,rwv,awv:',
     >    sngl(max(0.d0,vlib(2)*gamma)),sngl(rlib(2)),sngl(alib(2))
c       Real surface geometry parameters are the same as for the imaginary surface potential.
	  write(*,*) 'Real surface vd,rvd,avd:',
     >    sngl(vlib(3)*gamma),sngl(rlib(3)),sngl(alib(3))
	  write(*,*) 'Imag surface wd,rwd,awd:',
     >    sngl(max(0.d0,vlib(4)*gamma)),sngl(rlib(4)),sngl(alib(4))
	  write(*,*) 'Real SO pot vso,rvs,avs:',
     >    sngl(vlib(5)*gamma),sngl(rlib(5)),sngl(alib(5))
	  write(*,*) 'Imag SO pot wso,rws,aws:',
     >    sngl(vlib(6)*gamma),sngl(rlib(6)),sngl(alib(6))
        write(*,*) 
	ENDIF
...
C    >    ' E=',el,' DEF:',(betass(j),j=2,iqm,2)
     ...
     >                     dble(rv *atar**(1./3.)),dble(av),1)   
       ...
       write(*,*) ' VOL INTEGRAL =',rvolint,' SPH=',rvolintsph
...
C
       if(modtyp.gt.2) return
        ...
        if(AS.ne.0.d0 .and. AHF.ne.0.d0 .and. AAV.ne.0.d0) then
          write(35,'(f7.3,f9.3,f6.3,f6.3,f7.3,
...
ccc     New type support.   dmart 06.10.2021      
      if(imodel.eq.5)modelz='deform soft' 
