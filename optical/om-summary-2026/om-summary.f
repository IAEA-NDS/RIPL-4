...
c
c     INPUT FILE
c       omparameter.dat = RIPL Optical Model Potential (OMP) file
...
c
      ...
      character*1 ians
C     common/clas/nsph,nccvib,nccrot,nccsof,ndisp,nhe
      common/clas/nsph,nccvib,nccrot,nccsof,ndisp,nhe,nccrSRM,nccsSRM,
     > nneut, nprot, nalph, ndeut, ntrit,	nhe3 
...
c     Input option for sorting
     ...
     + /" Otherwise, sorting is on RIPL reference number.")')
      ians='n'
      read(5,'(a1)')ians
      if(ians.eq.'Y')ians='y'
c
 ...
 200  n1=i1
      ...
      write(*,*)'Number of neutron OMPs    = ',nneut
      write(*,*)'Number of proton OMPs     = ',nprot
      write(*,*)'Number of alpha OMPs      = ',nalph
      write(*,*)'Number of deuteron OMPs   = ',ndeut
      write(*,*)'Number of tritium OMPs    = ',ntrit
      write(*,*)'Number of He-3 OMPs       = ',nhe3
     ...
     &  nccrot      
      write(*,*)'Number of coupled channel spherical SRM potentials = ',                          
     &  nccsof      
      write(*,*)'Number of coupled channel multi-band RRM potentials= ',                          
     &  nccrSRM      
      write(*,*)'Number of coupled channel deformed SRM potentials  = ',                          
     &  nccsSRM      
...
c
c     Zero arrays
      call setr(0.d0,epot,6*ndim1)
      call setr(0.d0,rco,6*ndim1*ndim2)
      call setr(0.d0,aco,6*ndim1*ndim2)
      call setr(0.d0,pot,6*ndim1*ndim3)
c
      ieof=0
...
C     CC harmonic oscillator (vibrational)
      nccvib = nccvib + 1    
      read(ki,*) nisotop
      do n=1,nisotop
...
C       Reading soft rotor hamiltonian
 ...
 170  if(imodel.ne.4)go to 180
C	CC multi-band rigid rotor
      nccrSRM = nccrSRM + 1          
      read(ki,*) nisotop
      do n=1,nisotop
        read(ki,*) iz(n),ia(n),ncoll(n),lmax(n),idef(n),bandk(n),
     +    (def(n,k),k=2,idef(n),2)
        nlvmax = 0
C       do 152 k=1,ncoll(n)
C         read(ki,*) exv(k,n),spinv(k,n),iparv(k,n),
...
C         write(*,*) iz(n),ia(n),ex(k,n)
 ...
 180  if(imodel.ne.5)go to 200
C	CC multi-band rigid rotor
      nccsSRM = nccsSRM + 1          
      read(ki,*) nisotop
      do n=1,nisotop
        read(ki,*) iz(n),ia(n),ncoll(n),lmax(n),idef(n),bandk(n), 
C                  Isotope atomic and mass number read
     +    (def(n,k),k=2,idef(n),2) ! also deformations are read
...
C    +                 defv(k,n),defr(k,n),         
 ...
 200  continue
      ...
      return
 999  ieof=1
      write(6,*)'End of RIPL-4 optical potential library reached.'
      return
      end
...
c
C     parameter (ndim8=2000)
C     parameter (ndim8=50)
...
C     if(imodel.eq.0) nsph   = nsph   + 1
C     if(imodel.eq.1) nccrot = nccrot + 1            
C     if(imodel.eq.2) nccvib = nccvib + 1      
C     if(imodel.eq.3) then
C       write (*,*) 'nccsof =', nccsof             
C       write (*,*) 'nccrSRM =', nccrSRM             
C       write (*,*) 'nccsSRM =', nccsSRM             
C       pause 'type 3'
C	endif
C     if(imodel.eq.4) then
C       write (*,*) 'nccsof =', nccsof             
C       write (*,*) 'nccrSRM =', nccrSRM             
C       write (*,*) 'nccsSRM =', nccsSRM             
C       pause 'type 4'
C     endif
C     if(imodel.eq.5) then
C       write (*,*) 'nccsof =', nccsof             
C       write (*,*) 'nccrSRM =', nccrSRM             
C       write (*,*) 'nccsSRM =', nccsSRM             
C       pause 'type 5'
C     endif
C     if(imodel.eq.5) nccsSRM = nccsSRM + 1            
      if(idr.ne.0) ndisp = ndisp + 1                        
      if(emin.lt.1 .and. emax.gt.99) nhe = nhe + 1       
c
c     Get standard ID number
      do n=1,6
       if(izproj.eq.0.and.iaproj.eq.1) then
	   idx(i1)=1
         nneut = nneut + 1
	   exit
       endif
       if(izproj.eq.1.and.iaproj.eq.1) then
	   idx(i1)=2
         nprot = nprot + 1
	   exit
       endif
       if(izproj.eq.2.and.iaproj.eq.4) then
	   idx(i1)=6
	   nalph = nalph + 1
	   exit
       endif
       if(izproj.eq.1.and.iaproj.eq.2) then 
	   idx(i1)=3
         ndeut = ndeut + 1 
	   exit
       endif
       if(izproj.eq.1.and.iaproj.eq.3) then 
	   idx(i1)=4
	   ntrit = ntrit + 1
	   exit
       endif
       if(izproj.eq.2.and.iaproj.eq.3) then
	   idx(i1)=5
	   nhe3  = nhe3 + 1 
	   exit
       endif
...
c
C     parameter (ndim8=2000,jcount=40)
      parameter (jcount=40)
...
C     parameter (ndim8=50)
     ...
     3 lrefx(ndim8),irelx(ndim8),disrt(ndim8),djsrt(ndim8)
      common/bcdx/authx(80,ndim8),referx(80,ndim8),summryx(320,ndim8)
      dimension proj(6),imod(6)
      data nct/100000/
      data (proj(id),id=1,6)/' n ',' p ',' d ',' t ','3He','4He'/
      data com/','/,da/'-'/,bl/' '/
      data (imod(i),i=1,6)/'spher. ','CC rig.','CC vib.',
     > 'CC soft','CC softR','CC softD'/
c
  1   format('Lib.  Inc.  Model   Disp  Rel Z-Range  A-Range    E-Range
     +   Ref.   First ',/
     +       ' No. Part.   Type   Pot   Pot                      (MeV)
     +    No.   Author ',/)
  2   format(i4,2x,a3,3x,a8,2x,a3,2x,a3,i4,a1,i2,i5,a1,i3,f6.1,a1,f5.1,
     + i6,2x,16a1)
  3   format('                       REFERENCES'/)
  4   format(i3,'.  ',66a1,/6x,66a1,/6x,66a1)
  5   format(i4,2x,a3,3x,a8,2x,a3,2x,a3,i4,a1,i3,i4,a1,i3,f6.1,a1,f5.1,
     + i6,2x,16a1)
c
...
c
 ...
 100  ict=ict+1
      ...
      write(2,2)irefx(i1),proj(id),imod(im),dsp,rel,izminx(i1),da,
     1 izmaxx(i1),iaminx(i1),da,iamaxx(i1),eminx(i1),da,emaxx(i1),
     2 krefx(k1),(authx(j,i1),j=1,nauthx(i1))
      go to 112
 110  write(2,5)irefx(i1),proj(id),imod(im),dsp,rel,izminx(i1),da,
     1 izmaxx(i1),iaminx(i1),da,iamaxx(i1),eminx(i1),da,emaxx(i1),
     2 krefx(k1),(authx(j,i1),j=1,nauthx(i1))
...
c
      ...
      if(imodelx(i1).eq.4)model='CC multi-band rigid rotor model'
      if(imodelx(i1).eq.5)model='CC deformed soft rotor model'
